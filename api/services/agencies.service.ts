import { AgenciesModel } from '../models/socialMediaAgency.model';
...
export class AgenciesService {
  ...
  async getAgencyByUser(userId: string) {
    return AgenciesModel.findOne({ userId });
  }

  async getCount() {
    return AgenciesModel.countDocuments();
  }

  async getAllAgencies() {
    return AgenciesModel.find();
  }

  async getAllAgenciesSlug() {
    return AgenciesModel.find({}, 'slug');
  }

  async getAgencyInformation(agencyId: string) {
    return AgenciesModel.findById(agencyId);
  }

  async approveOrDecline(email: string, action: string, id: string) {
    await AgenciesModel.findByIdAndUpdate(id, { approved: action === 'approve' });
    const agency = await AgenciesModel.findById(id).populate('user');
...
<body style="font-family: Arial, sans-serif; margin: 0; padding: 0;">
  ...
  async createAgency(userId: string, body: CreateAgencyDto) {
    const agency = await AgenciesModel.create({ ...body, userId });
    await this.notificationService.sendEmail(
      'nevo@postiz.com',
      'New agency created',
      `<html lang="en">
